...
def render_scroll_depth(sec_summary, page_key="home"):
    ...
    """
    st.subheader("📊 섹션 노출 뎁스")
    st.caption(
        "각 섹션의 **노출 효율**을 측정해요. 1번 섹션의 순 노출자를 100%로 잡고, "
        "페이지 아래 섹션들이 얼마나 노출됐는지 비율로 표시. "
        "**컴포넌트의 노출 발화 효율**도 영향을 주므로, 사용자 도달률을 보고 싶다면 → **스크롤 뎁스** 탭 참고."
    )
...
def render_user_reach_depth(sections_df, user_sec_df, page_key="home"):
    ...
    """
    st.subheader("📜 스크롤 뎁스 (사용자 도달 기준)")
    st.caption(
        "사용자가 **페이지 어디까지 내려갔는지** 측정해요. "
        "각 사용자의 가장 깊은 섹션을 찾아 'N번 이상까지 도달한 사용자' 비율을 누적 표시. "
        "**단조 감소 곡선** — 깊어질수록 인원은 절대 늘 수 없음. "
        "(섹션의 노출 효율을 보고 싶다면 → **섹션 노출 뎁스** 탭 참고)"
    )
...
def render_dashboard(page_config: dict):
    ...
    # ════════════════════════════════════════════
    # TAB 4 — 스크롤 뎁스 (사용자 단위 누적 Funnel: 페이지 어디까지 도달했나)
    # ════════════════════════════════════════════
    with tab4:
